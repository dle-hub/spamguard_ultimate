# SpamGuard Ultimate - Kod Açıklamaları

Bu belge, `spamguard_ultimate.xml` eklenti dosyasının içerdiği kodları ve bu kodların işlevlerini açıklamaktadır.

## Veritabanı Yapısı (SQL)

Eklenti, çalışması için iki yeni veritabanı tablosu oluşturur ve yönetici paneline bir menü öğesi ekler.

### `_spam_logs`
Bu tablo, eklenti tarafından engellenen tüm spam girişimlerini kaydeder. IP adresi, tarih, kullanıcı adı (varsa), engelleme nedeni gibi bilgileri içerir.

```sql
CREATE TABLE IF NOT EXISTS `{prefix}_spam_logs` (
...
### `_spam_whitelist`
Bu tablo, yanlışlıkla engellenebilecek belirli IP adreslerini, e-posta adreslerini veya alan adlarını beyaz listeye eklemek için kullanılır.

```sql
CREATE TABLE IF NOT EXISTS `{prefix}_spam_whitelist` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `data` varchar(255) NOT NULL,
  `type` enum('ip','email','domain') NOT NULL DEFAULT 'ip',
...
### `_admin_sections`
Bu SQL sorgusu, DLE yönetici paneline "SpamGuard Ultimate" için bir menü öğesi ekler.

```sql
INSERT INTO `{prefix}_admin_sections` (`name`, `title`, `descr`, `icon`, `allow_groups`) VALUES
('spamguard', 'SpamGuard Ultimate', 'Gelişmiş Anti-Spam ve Güvenlik Yönetimi', 'blockip.png', '1');
```
...
Eklenti, işlevselliğini sağlamak için aşağıdaki PHP dosyalarını kullanır.

### 1. `engine/inc/spamguard.php`

Bu dosya, eklentinin yönetici panelindeki ana arayüzünü oluşturur. Yöneticilerin eklenti ayarlarını yapılandırmasına, engellenen IP'leri ve kullanıcıları yönetmesine, spam loglarını görüntülemesine ve kullanıcı analizleri yapmasına olanak tanır.

**İşlevleri:**
-   **Dashboard:** Genel istatistikleri (toplam üye, yasaklı IP, engellenen spam vb.) gösterir.
-   **Genel Ayarlar:** Eklentiyi aktif/pasif etme, StopForumSpam API anahtarını girme, Proxy/VPN engelleme gibi temel ayarları içerir.
-   **Mail Servisleri:** Yasaklanacak geçici e-posta servislerinin alan adlarını yönetir.
-   **Yasaklı Liste:** Manuel olarak IP adresi engelle'yi ve engellenen IP'leri listelemeyi sağlar.
-   **Kullanıcı Analizi:** Üyeleri risk puanlarına göre analiz eder ve şüpheli hesapları (Ghost user, pasif kullanıcı vb.) tespit etmeye yardımcı olur.
-   **Loglar:** Engellenen tüm spam faaliyetlerinin kaydını gösterir.
...
echo "<div id='banned_tab' class='panel panel-flat tab-content-panel' style='{$style_banned}'><div class='panel-heading'><h5 class='panel-title'>Yasaklı IP Listesi</h5><div class='heading-elements'><ul class='icons-list'><li><a href='#' onclick="$('#newblock').modal(); return false;"><i class='fa fa-plus-circle'></i> Ekle</a></li></ul></div></div>";
...
while($row = $db->get_row()) {
     ...
     $show_name = "-";
     if($row['username']) $show_name = "<span class='text-semibold text-primary'>{\$row['name']}</span>";
...
while($log = $db->get_row()) { echo "<tr><td>".date("d.m.Y H:i",$log['date']).